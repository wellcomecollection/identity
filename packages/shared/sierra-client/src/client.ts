...
export default class SierraClient {
  ...
  async getPatronRecordByBarcode(
    barcode: string
  ): Promise<APIResponse<PatronRecord>> {
    return this.getInstance().then((instance) => {
      return instance
        ...
        .catch((error) => {
          if (error.response) {
            switch (error.response.status) {
              case 404:
                return errorResponse(
                  'Patron record with barcode [' + barcode + '] not found',
                  ResponseStatus.NotFound,
                  error
                );
            }
  ...
  async createPatronRecord(
    ...
    return this.getInstance().then((instance) => {
      return instance
        .post('/patrons', toCreatePatron(firstName, lastName, pin), {
          validateStatus: (status) => status === 200,
        })
        .then((response) =>
            ...
            switch (error.response.status) {
              case 400: {
                if (
                  ...
                  return errorResponse(
                    'Malformed or invalid Patron record creation request',
  ...
  async updatePatronPostCreationFields(
    recordNumber: number,
    email: string
  ): Promise<APIResponse<PatronRecord>> {
    return this.getInstance().then((instance) => {
