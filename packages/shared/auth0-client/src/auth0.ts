...
export function toAuth0Profile(auth0User: any): Auth0Profile {
  ...
  if (
    !isNonBlank(userIdStr) ||
    !isNonBlank(name) ||
    !isNonBlank(email) ||
    !isNonBlank(creationDate)
  ) {
    throw new Error(
      'One or more required UserProfile fields are missing. Have all necessary scopes been requested?'
    );
  }
...
export function toAuth0SearchResults(
  page: number,
  sort: string,
  sortDir: number,
...
function extractUserId(value: string): string {
  if (value.startsWith(SierraUserIdPrefix)) {
    const userId: string = value.slice(
      value.indexOf(SierraUserIdPrefix) + SierraUserIdPrefix.length
    );
    if (isNaN(Number(userId))) {
      throw new Error(
        'Invalid user ID field, cannot extract numerical ID from [' +
          value +
          ']'
      );
    }
